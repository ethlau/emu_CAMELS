...
#mass=np.array([12.15,12.7,13.55])
#mass_str=np.array(['12-12.3','12.3-13.1','13.1-14.0'])

mass_str =['12.0-12.3','12.3-12.7','12.7-13.2', '13.2-14.0']
...
def load_xsb_profiles(home,suite,sims,snap,mass_str,profiles=profiles):
    ...
    xp = 10**np.linspace(-2.5, 0.5, num=20) 
    ...
    for s in np.arange(len(sims)):
        for n in np.arange(len(snap)):
            for m in np.arange(len(mass_str)):
                ...
                #f=home+suite+'/old_profiles/'+suite+'_'+sims[s]+'_'+snap[n]+'_uw_'+mass_str[m]+'.txt'
                f=home+suite+'/'+suite+'_'+sims[s]+'_'+snap[n]+'_uw_'+mass_str[m]+'.txt'
                (x,  
                ...
                for p in profiles:
                    ...
                    #if p == 'hot_density' or p == 'xsb' :
                    if True:
                        temp_y = np.interp(np.log10(xp), np.log10(x), np.log10(yi))
                        temp_errup  = np.interp(np.log10(xp), np.log10(x), np.log10(errupi))
                        temp_errlow = np.interp(np.log10(xp), np.log10(x), np.log10(errlowi))
                        temp_stddev = np.interp(np.log10(xp), np.log10(x), np.log10(stddevi))
                        temp_y_med = np.interp(np.log10(xp), np.log10(x), np.log10(ymedi))

                    if False:
                        temp_y = np.interp(xp, x, (yi))
                        temp_errup  = np.interp(xp, x, errupi)
                        temp_errlow = np.interp(xp, x, errlowi)
                        temp_stddev = np.interp(xp, x, stddevi)
                        temp_y_med = np.interp(xp, x, ymedi)
    ...
    for p in profiles:
        ...
        up_profiles[p] = np.nan_to_num(np.array(up_profiles[p]))
        dn_profiles[p] = np.nan_to_num(np.array(dn_profiles[p]))
        std_profiles[p] = np.nan_to_num(np.array(std_profiles[p]))
        median_profiles[p] = np.nan_to_num(np.array(median_profiles[p]))
 

    return  xp, mean_profiles, up_profiles, dn_profiles, std_profiles, median_profiles
